...
// scope on our side.
async function createSubscriptionCheckout({ customer, baseUrl }) {
  const { price } = await ensureBetaProduct();
  const stripeCustomer = await getOrCreateStripeCustomer(customer);
  // Return URLs go to the CUSTOMER-side dashboard (not /admin) — the
  // recipient of the checkout link is the campaign manager, who logs
  // in via /login, not /admin/login.
  const session = await stripe().checkout.sessions.create({
    mode: 'subscription',
    customer: stripeCustomer.id,
    line_items: [{ price: price.id, quantity: 1 }],
    success_url: `${baseUrl}/dashboard/billing/return?status=success&session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${baseUrl}/dashboard/billing/return?status=cancel`,
    subscription_data: {
      metadata: {
