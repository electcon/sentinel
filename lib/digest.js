...
const FROM_EMAIL = process.env.DIGEST_FROM_EMAIL || 'Sentinel <digest@sentinel.parallaxadvisory.llc>';
...
function buildEmail(data) {
  ...
  const reviewPending = data.reviewQueue?.pending || 0;
  const subjectBits = [];
  if (data.openThreats > 0) subjectBits.push(`${data.openThreats} open threat${data.openThreats === 1 ? '' : 's'}`);
  if (reviewPending > 0) subjectBits.push(`${reviewPending} to review`);
  const subject = subjectBits.length
    ? `[${subjectBits.join(' · ')}] Sentinel digest — ${dateStr}`
    : `Sentinel digest — ${dateStr} — ${data.totalMentions} mentions`;

  const dashboardUrl = process.env.DASHBOARD_BASE_URL || 'https://sentinel-staging-i3ug.onrender.com';
  const text = [
    ...
    `Review queue (pending Tier-2): ${reviewPending}`,
    '',
    'By tier:',
    ...
    '',
    ...(reviewPending > 0 ? [
      `Review queue — ${reviewPending} mention${reviewPending === 1 ? '' : 's'} pending triage:`,
      ...((data.reviewQueue?.oldest || []).map(m => [
        `  ${m.target} (${m.source})`,
        `       ${m.source_url}`,
        `       ${(m.body_excerpt || '').slice(0, 200)}`,
        ''
      ].join('\n'))),
      `  Open: ${dashboardUrl}/dashboard/review-queue`,
      ''
    ] : []),
    'Open the dashboard:',
...
${reviewPending > 0 ? `<div style="background:#fff7e6;border-left:3px solid #d8902f;padding:12px 14px;margin-bottom:18px;font-size:14px">
<strong>${reviewPending} Tier-2 mention${reviewPending === 1 ? '' : 's'} pending review.</strong> Hostile rhetoric the classifier flagged for human triage. <a href="${escapeHtml(dashboardUrl)}/dashboard/review-queue">Open the review queue</a>.
</div>` : ''}
