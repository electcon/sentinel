...
const FROM_EMAIL = process.env.ALERT_FROM_EMAIL || 'Sentinel <alerts@sentinel.parallaxadvisory.llc>';
...
function buildEmail(p) {
  ...
  const eventUrl = p.eventId ? `${dashboardBase}/dashboard/threats/${p.eventId}` : null;
  // One-click acknowledgment link — signed token, no login required.
  // Marks status='reviewing' + records the time. Useful at 3am.
  let ackUrl = null;
  if (p.eventId) {
    try {
      const { signActionToken } = require('./auth');
      const tok = signActionToken({ kind: 'threat_ack', id: p.eventId, ttlMs: 14 * 24 * 60 * 60 * 1000 });
      ackUrl = `${dashboardBase}/threat-ack/${encodeURIComponent(tok)}`;
    } catch (_) { /* SESSION_SECRET unset locally; omit ack link */ }
  }
  ...
  const text = [
    ...
    '',
    ...(ackUrl ? [`One-click acknowledge (marks as reviewing, no login):`, `  ${ackUrl}`, ''] : []),
    ...(eventUrl ? [`Open in dashboard: ${eventUrl}`, ''] : []),
...
${ackUrl ? `<div style="display:flex;gap:10px;margin-bottom:18px">
<a href="${escapeHtml(ackUrl)}" style="background:#3a9c3a;color:#fff;padding:10px 18px;border-radius:5px;text-decoration:none;font-weight:600;font-size:14px">✓ Acknowledge (no login)</a>
${eventUrl ? `<a href="${escapeHtml(eventUrl)}" style="background:#1c2330;color:#e6edf3;padding:10px 18px;border-radius:5px;text-decoration:none;font-weight:500;font-size:14px">Open in dashboard</a>` : ''}
</div>` : ''}
...
// Routes by `channel`: 'email' (default) | 'webhook' | 'slack'.
async function sendThreatAlert(payload) {
  if (payload.channel === 'webhook') return sendThreatWebhook(payload);
  if (payload.channel === 'slack') return sendThreatSlack(payload);
  return sendThreatEmail(payload);
}

// Native Slack incoming-webhook destination. Customer pastes a
// https://hooks.slack.com/services/T.../B.../xxxx URL; Sentinel posts the
// Slack-formatted attachment payload directly. URL is the secret — no
// HMAC needed (Slack treats the path as a bearer credential).
//
// Attachments payload mirrors examples/slack-webhook.js so customers
// who already deployed the receiver pattern get the same look once they
// migrate to native.
function buildSlackPayload(p) {
  const tier = p.tier;
  const color = tier === 4 ? '#7a1019' : tier === 3 ? '#d8902f' : '#999999';
  const tag = tierLabel(tier);
  const dashboardBase = process.env.DASHBOARD_BASE_URL || 'https://sentinel.parallaxadvisory.llc';
  const eventUrl = p.eventId ? `${dashboardBase}/dashboard/threats/${p.eventId}` : null;
  const ts = (() => {
    const t = p.mention?.posted_at ? Date.parse(p.mention.posted_at) : Date.now();
    return Math.floor((isNaN(t) ? Date.now() : t) / 1000);
  })();
  return {
    attachments: [{
      color,
      title: `${tag} — ${p.target?.name || 'unknown target'}`,
      title_link: p.mention?.source_url || undefined,
      text: String(p.mention?.body_excerpt || '').slice(0, 1500),
      fields: [
        { title: 'Source', value: p.mention?.source || 'unknown', short: true },
        { title: 'Author', value: p.mention?.author_handle || 'unknown', short: true },
        { title: 'Customer', value: p.customer?.name || 'unknown', short: true },
        { title: 'Posted', value: p.mention?.posted_at || 'unknown', short: true }
      ],
      footer: p.rationale ? `Sentinel · ${String(p.rationale).slice(0, 200)}` : 'Sentinel',
      ts,
      ...(eventUrl ? {
        actions: [{ type: 'button', text: 'Open in Sentinel', url: eventUrl, style: 'primary' }]
      } : {})
    }]
  };
}

async function sendThreatSlack(payload) {
  const url = payload.destination;
  if (!url) return { ok: false, error: 'no slack webhook URL' };
  if (!/^https:\/\/hooks\.slack\.com\/services\//i.test(url)) {
    return { ok: false, error: 'expected https://hooks.slack.com/services/... URL' };
  }
  ...
  try {
    const r = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'User-Agent': 'Sentinel/0.1 slack' },
      body,
      signal: ctrl.signal
    });
    clearTimeout(t);
    if (!r.ok) {
      ...
      return { ok: false, error: `slack ${r.status}: ${text.slice(0, 200)}`, channel: 'slack' };
    }
    return { ok: true, channel: 'slack', status: r.status };
  } catch (e) {
    clearTimeout(t);
    return { ok: false, error: e.name === 'AbortError' ? 'slack timeout' : e.message, channel: 'slack' };
  }
}
...
async function sendThreatWebhook(payload) {
  ...
  const envelope = {
    ...
    dashboard_url: (process.env.DASHBOARD_BASE_URL || 'https://sentinel.parallaxadvisory.llc') + '/dashboard'
  };
...
module.exports = { sendThreatAlert, sendThreatEmail, sendThreatWebhook, sendThreatSlack, buildEmail, buildSlackPayload, generateWebhookSecret, FROM_EMAIL };
