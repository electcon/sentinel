...
function requireCustomerAuth(pool) {
  return async function (req, res, next) {
    ...
    try {
      const r = await pool.query('SELECT id, name, alert_email, digest_email, status, must_change_password FROM customers WHERE id = $1', [sess.customerId]);
      if (!r.rowCount) {
      ...
      // First-login password change is required: lock the customer to
      // /dashboard/force-password-change until they set their own.
      // Allow logout, allow the change-password POST, and allow the
      // page itself.
      if (req.customer.must_change_password) {
        const allow = req.path === '/dashboard/force-password-change'
          || req.path === '/logout'
          || req.path === '/dashboard/force-password-change/save';
        if (!allow) return res.redirect('/dashboard/force-password-change');
      }
      next();
    } catch (e) {
      next(e);
...
// One-click action tokens (e.g. "acknowledge this threat from the alert
// email"). HMAC-signed payload + expiry. Same secret as session cookies.
// Format: base64url(JSON{kind,id,exp}).<base64url(hmac)>
function signActionToken({ kind, id, ttlMs = 7 * 24 * 60 * 60 * 1000 }) {
  if (!SESSION_SECRET) throw new Error('SESSION_SECRET not set');
  const payload = JSON.stringify({ k: kind, i: id, e: Date.now() + ttlMs });
  const b64 = Buffer.from(payload).toString('base64url');
  const sig = crypto.createHmac('sha256', SESSION_SECRET).update(b64).digest('base64url');
  return `${b64}.${sig}`;
}

function verifyActionToken(token, expectedKind) {
  if (!token || !SESSION_SECRET) return null;
  ...
  if (!payload || !payload.k || !payload.i || !payload.e) return null;
  if (expectedKind && payload.k !== expectedKind) return null;
  if (payload.e < Date.now()) return null;
  return { kind: payload.k, id: payload.i, expiresAt: payload.e };
}
