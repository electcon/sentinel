...
// twitterapi.io free tier QPS limit is 1 req per 5 seconds. Pace
// requests at the lib level so all callers (worker, smoke endpoint)
// share the budget. Override with X_QPS_GAP_MS once on a paid plan.
const QPS_GAP_MS = parseInt(process.env.X_QPS_GAP_MS, 10) || 5100;
let _lastCallAt = 0;
let _waitChain = Promise.resolve();
function _rateLimitTicket() {
  // Serialize via a chained promise so concurrent callers each get
  // their own slot. Without this, two parallel calls would both see
  // the same _lastCallAt and overlap.
  const next = _waitChain.then(async () => {
    const elapsed = Date.now() - _lastCallAt;
    if (elapsed < QPS_GAP_MS) {
      await new Promise(res => setTimeout(res, QPS_GAP_MS - elapsed));
    }
    _lastCallAt = Date.now();
  });
  _waitChain = next.catch(() => {});
  return next;
}
