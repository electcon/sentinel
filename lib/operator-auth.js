...
function requireOperator(pool) {
  return async function (req, res, next) {
    ...
    if (sess) {
      // Special bootstrap session — created by /admin/login bootstrap-tab
      // submission. operatorId is the literal string 'bootstrap' (not a
      // UUID), so don't try to look it up in operators.
      if (sess.operatorId === 'bootstrap') {
